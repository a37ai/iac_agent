...
def build_k8s_client(credentials: Dict[str, Any]) -> kubernetes.client.ApiClient:
    """
    Build a Kubernetes ApiClient from given credentials. Two supported modes:
      A) {"kubeconfig_path": "/path/to/kubeconfig"} or {"kubeconfig_file_content": "<kubeconfig file content as string>"}
      B) {
          "host": "https://k8s.example.com", 
          "bearer_token": "my-token",
          "certificate_authority": "/path/to/ca.crt",
    ...
    elif "kubeconfig" in credentials:
        config_file = StringIO(credentials["kubeconfig"]["content"])
        k8s_config.load_kube_config_from_dict(yaml.safe_load(config_file))
        return k8s_client.ApiClient()

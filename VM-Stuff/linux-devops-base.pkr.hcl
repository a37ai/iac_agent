...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      # -----------------------------------------------------------------
      # 3. Azure CLI (If enabled)
      # -----------------------------------------------------------------
      "if [ \"$(jq -r .azure.enabled /tmp/agent-config.json)\" = \"true\" ]; then",
      "  echo '--- Installing Azure CLI (latest) ---'",
      "  sudo rpm --import https://packages.microsoft.com/keys/microsoft.asc",
      "  sudo curl -sSL -o /etc/yum.repos.d/azure-cli.repo https://packages.microsoft.com/config/rhel/7/azure-cli.repo",
      "  sudo yum install -y azure-cli",
      "",
      "  echo 'Writing Azure SP credentials to /opt/agent/azure...'",
      "  mkdir -p /opt/agent/azure",
      "  AZ_TENANT=$(jq -r .azure.tenant_id /tmp/agent-config.json)",
      "  AZ_CLIENT=$(jq -r .azure.client_id /tmp/agent-config.json)",
      "  AZ_SECRET=$(jq -r .azure.client_secret /tmp/agent-config.json)",
      "  AZ_SUB=$(jq -r .azure.subscription_id /tmp/agent-config.json)",
      "  cat <<EOF >/opt/agent/azure/sp_credentials.txt",
      "tenant_id = $AZ_TENANT",
      "client_id = $AZ_CLIENT",
      ...
      "  cd /tmp",
      "  CHEF_URL=\"https://packages.chef.io/files/stable/chef-workstation/${var.chef_workstation_version}/el/7/chef-workstation_${var.chef_workstation_version}-1.el7.x86_64.rpm\"",
      "  curl -sSL \"$CHEF_URL\" -o chef.rpm",
      "  sudo rpm -Uvh chef.rpm",
      "  rm -f chef.rpm",
      "",
      "  echo 'Storing Chef keys if provided...'",
      "  mkdir -p /opt/agent/chef",
      "  CHEF_CLIENT_KEY=$(jq -r .chef.client_key /tmp/agent-config.json)",
      "  if [ \"$CHEF_CLIENT_KEY\" != \"null\" ]; then",
      "    echo \"$CHEF_CLIENT_KEY\" >/opt/agent/chef/client.pem",
      "  fi",
      "  CHEF_VALIDATION_KEY=$(jq -r .chef.validation_key /tmp/agent-config.json)",
      "  if [ \"$CHEF_VALIDATION_KEY\" != \"null\" ]; then",
      "    echo \"$CHEF_VALIDATION_KEY\" >/opt/agent/chef/validation.pem",
      "  fi",
      "fi",

...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      # -----------------------------------------------------------------
      # Only run if kubernetes.enabled == true
      ...
      "  cd /tmp",
      # Embed the var.terraform_version directly:
      ...
      "  cd /tmp",
      # Similarly, embed the var.helm_version directly:
      ...
      #   # 9c. Chef
      #   "if [ \"$(jq -r .chef.enabled /tmp/agent-config.json)\" = \"true\" ]; then",
      #   "  echo \"--- Installing Chef Workstation version ${var.chef_workstation_version} ---\"",
      #   "",
      #   "  # Navigate to /tmp directory",
      #   "  cd /tmp",
      #   "",
      #   "  # Define Chef Workstation version",
      #   "  CHEF_VERSION=${var.chef_workstation_version}",
      #   "",
      #   "  # Define Chef Workstation download URL using Chef's Download API",
      #   "  # PLATFORM: 'el' for Enterprise Linux (RHEL/CentOS/Amazon Linux)",
      #   "  # PLATFORM_VERSION: '7' for RHEL 7 (Amazon Linux 2 is based on RHEL 7)",
      #   "  # ARCHITECTURE: 'x86_64'",
      #   "",
      #   "  CHEF_URL=\"https://chefdownload-commercial.chef.io/stable/chef-workstation/download?p=el&pv=7&m=x86_64&v=${CHEF_VERSION}&license_id=${var.chef_license_id}\"",
      #   "",
      #   "  echo \"Downloading Chef Workstation from $CHEF_URL\"",
      #   "",
      #   "  # Download the Chef Workstation RPM",
      #   "  wget -O chef.rpm \"$CHEF_URL\" || {",
      #   "    echo \"Failed to download Chef Workstation RPM from $CHEF_URL\";",
      #   "    exit 1;",
      #   "  }",
      #   "",
      #   "  # Verify that the downloaded file is a valid RPM",
      #   "  if rpm -K chef.rpm >/dev/null 2>&1; then",
      #   "    echo \"Chef Workstation RPM verification succeeded.\";",
      #   "  else",
      #   "    echo \"Chef Workstation RPM verification failed. The downloaded file may be invalid.\";",
      #   "    cat chef.rpm", // Output the contents for debugging (likely HTML error page)
      #   "    exit 1;",
      #   "  fi",
      #   "",
      #   "  # Install Chef Workstation using YUM",
      #   "  sudo yum localinstall -y chef.rpm || {",
      #   "    echo \"Chef Workstation installation failed.\";",
      #   "    exit 1;",
      #   "  }",
      #   "",
      ...
      #   "",
      # "  # Store Chef Service Principal credentials securely",
      # "  echo 'Storing Chef credentials...'",
      # "  sudo mkdir -p /opt/agent/chef",
      # "",
      # "  CHEF_CLIENT_KEY=$(jq -r .chef.client_key /tmp/agent-config.json)",
      # "  if [ \"$CHEF_CLIENT_KEY\" != \"null\" ]; then",
      # "    echo \"$CHEF_CLIENT_KEY\" | sudo tee /opt/agent/chef/client.pem > /dev/null",
      # "  fi",
      # "",
      # "  CHEF_VALIDATION_KEY=$(jq -r .chef.validation_key /tmp/agent-config.json)",
      # "  if [ \"$CHEF_VALIDATION_KEY\" != \"null\" ]; then",
      # "    echo \"$CHEF_VALIDATION_KEY\" | sudo tee /opt/agent/chef/validation.pem > /dev/null",
      # "  fi",
      # "",
      # "  sudo chmod 600 /opt/agent/chef/client.pem /opt/agent/chef/validation.pem || true",
      # "",
      # "  echo 'Chef credentials stored successfully.'",
      #   "fi",

      # -----------------------------------------------------------------
      # 10. Jenkins (If enabled)
      # -----------------------------------------------------------------
      # Only run if jenkins.enabled == true
      "#!/bin/bash",
      ...
      "  # Enable and start Jenkins using systemctl",
      ...
      "  DD_API=$(jq -r .datadog.api_key /tmp/agent-config.json)",
      "  DD_APP=$(jq -r .datadog.app_key /tmp/agent-config.json)",
      "  echo \"api_key = $DD_API\"   >/opt/agent/datadog/datadog_keys.txt",
      "  echo \"app_key = $DD_APP\"  >>/opt/agent/datadog/datadog_keys.txt",
      "fi",
      ...
      "  SPLUNK_HOST=$(jq -r .splunk.host /tmp/agent-config.json)",
      "  SPLUNK_PORT=$(jq -r .splunk.port /tmp/agent-config.json)",
      "  SPLUNK_USER=$(jq -r .splunk.username /tmp/agent-config.json)",
      "  SPLUNK_PASS=$(jq -r .splunk.password /tmp/agent-config.json)",
      "  cat <<EOF >/opt/agent/splunk/splunk_credentials.txt",
      "host=$SPLUNK_HOST",
      "port=$SPLUNK_PORT",
      ...
      "  ES_URL=$(jq -r .elasticsearchKibana.elasticsearch_url /tmp/agent-config.json)",
      "  ES_USER=$(jq -r .elasticsearchKibana.elasticsearch_user /tmp/agent-config.json)",
      "  ES_PASS=$(jq -r .elasticsearchKibana.elasticsearch_password /tmp/agent-config.json)",
      "  cat <<EOF >/opt/agent/elasticsearch/elasticsearch.txt",
      "elasticsearch_url=$ES_URL",
      "user=$ES_USER",
      "password=$ES_PASS",
      "EOF",
      "",
      "  KB_URL=$(jq -r .elasticsearchKibana.kibana_url /tmp/agent-config.json)",
      "  if [ \"$KB_URL\" != \"null\" ]; then",
      "    KB_USER=$(jq -r .elasticsearchKibana.kibana_user /tmp/agent-config.json)",
      "    KB_PASS=$(jq -r .elasticsearchKibana.kibana_password /tmp/agent-config.json)",
      "    cat <<EOF >/opt/agent/elasticsearch/kibana.txt",
      "kibana_url=$KB_URL",
      "user=$KB_USER",
      "password=$KB_PASS",
      "EOF",
      ...
      "  GF_URL=$(jq -r .grafana.url /tmp/agent-config.json)",
      "  GF_KEY=$(jq -r .grafana.api_key /tmp/agent-config.json)",
      "  cat <<EOF >/opt/agent/grafana/grafana_credentials.txt",
      "url=$GF_URL",
      "api_key=$GF_KEY",
      "EOF",
      "fi",
      ...
      "  cd /tmp",
      # Directly embed var.prometheus_version:
